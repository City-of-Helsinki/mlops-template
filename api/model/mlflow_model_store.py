...
class MlFlowModelStore(ModelStore):
    ...
    def __init__(self, model_name='model', model_version='latest',
        ...
        for i, n in enumerate(signature.inputs.input_names()):
            request_types.append({'name': n, 'type': signature.inputs.numpy_types()[i].type})

        response_types = []
        for i, n in enumerate(signature.outputs.input_names()):
            response_types.append({'name': n, 'type': signature.outputs.numpy_types()[i].type})

        # Schema for request (X)
        self.request_schema_class = self.__create_pydantic_model("DynamicApiRequest", request_types)
        # Schema for response (y)
        self.response_schema_class = self.__create_pydantic_model("DynamicApiResponse", response_types)
